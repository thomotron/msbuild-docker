...
RUN apt-get update && \
    ...
    apt-get update && \
    apt-get install --install-recommends --yes winehq-stable winbind cabextract dotnet-sdk-8.0 && \
...
COPY --chown=${USER_ID}:${GROUP_ID} ["Reference Assemblies", "/home/runner/.wine/drive_c/Program Files (x86)/Reference Assemblies"]
